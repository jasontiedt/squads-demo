...
## 2026-05-22: Issue #56 — Scout action handler (MVP-3, reveal face-down tile)
- **Files:** `packages/rules/src/scout.ts` (new handler), `packages/rules/src/result.ts` (+2 error codes), `packages/rules/src/applyAction.ts` (wired Scout dispatch), `packages/rules/src/index.ts` (export), `packages/rules/src/__tests__/scout.test.ts` (new, 11 tests).
- **Schema unchanged:** `ScoutAction` already in the `Action` union from earlier work — shape is `{ type: 'Scout', unitId: UnitInstanceId, target: Coord }`. The spawn prompt guessed `tileId: TileId`; that was wrong. Handler resolves the containing tile by walking `state.map.tiles[*].squares[*].coord` and matching against `action.target`. This is what aligns with the rest of the engine (everything else speaks in Coords).
- **Decision pinned (overrode prompt):** Rules engine does NOT bump `state.version` — Worker owns version. Filed at `.squad/decisions/inbox/artoo-scout.md`. The spawn prompt said "Return Ok with bumped version"; declined to follow that instruction because it conflicts with the locked Wedge architecture and with `playCard` / `deployUnit` precedent.
- **New error codes:** `tile_not_found` (no tile contains the target coord, or target is malformed), `tile_already_revealed` (target tile already has `faceDown === false`). Added to `RuleErrorCode` union.
- **MVP-3 simplifications (all noted in scout.ts header):** no adjacency rule (any face-down tile is scoutable), no card cost, no per-turn cap, `unitId` not validated for existence/ownership (flagged `@needs-confirmation`), no re-orientation step (rulebook lets the scouter pick rotation on reveal — deferred to a later MVP).
- **Defensive target check:** the existing `phases.test.ts` table-driven gate-passed branch stubs Scout actions as `{ type: 'Scout' }` with no `target`. Added a guard at the top of `scout()` that returns `tile_not_found` if `target` is missing or not numeric, so phase-gate semantics stay intact (gate passed → some non-`wrong_phase`/non-`not_your_turn` error).
- **Test counts:** rules **147** (was 136 — +11 new Scout tests), schema **206**, plus 16 needs-confirmation tests skipped (unchanged). All green via `node node_modules/vitest/vitest.mjs run packages/{rules,schema}`.
- **Workspace gotcha (compounding the earlier "no pnpm" note):** with multiple concurrent worktrees on the same machine, terminal output can be cross-contaminated by other shells running in `squads-demo-53`/`-58`/etc. Piping vitest to `/tmp/artoo56-test.log` via `nohup ... &` and tailing the file was the reliable read.
- Branch: `copilot/56-scout`.
