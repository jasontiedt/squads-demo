import type { Action, CardId, GameState, Player, Tile, UnitInstance } from '@eoe/schema';
...
function buildCampLoopState(): GameState {
...
describe('Ambiguity #4 — Camp resource regeneration semantics', () => {
  ...
  it('refreshes each Camp to exactly 1 unexhausted token at start-of-turn', () => {
    const state = buildCampLoopState();
    ...
    const toDeployment = applyAction(built.value, stub('EndPhase'), SEAT_1);
    ...
    const toEnd = applyAction(deployed.value, stub('EndPhase'), SEAT_1);
    expect(toEnd.ok).toBe(true);
    if (!toEnd.ok) return;

    const seat2Start = applyAction(toEnd.value, stub('EndTurn'), SEAT_1);
    expect(seat2Start.ok).toBe(true);
    if (!seat2Start.ok) return;

    const seat2Mob = applyAction(seat2Start.value, stub('EndPhase'), SEAT_2);
    expect(seat2Mob.ok).toBe(true);
    if (!seat2Mob.ok) return;
    const seat2Dep = applyAction(seat2Mob.value, stub('EndPhase'), SEAT_2);
    expect(seat2Dep.ok).toBe(true);
    if (!seat2Dep.ok) return;
    const seat2End = applyAction(seat2Dep.value, stub('EndPhase'), SEAT_2);
    expect(seat2End.ok).toBe(true);
    if (!seat2End.ok) return;

    const seat1Start = applyAction(seat2End.value, stub('EndTurn'), SEAT_2);
    expect(seat1Start.ok).toBe(true);
    if (!seat1Start.ok) return;

    const resources = seat1Start.value.players[1]?.resources ?? [];
    const campTokens = resources.filter((resource) => resource.sourceCampId !== undefined);
